...
# Individual Level Data Cleaning File
...
* Not necessary, it doesn't add countries to the sample and we only need a cross section for this study
...
# run without occ status codes for now
...
#   
# 
 nl87 <- dplyr::select(nl87, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, inclarge, isco88, isco88_2digit, fisco88_2digit, misco88_2digit, spisco88_2digit)
...
# 
# 
 ie87 <- dplyr::select(ie87, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, inclarge, isco88, isco88_2digit, fisco88_2digit, misco88_2digit, spisco88_2digit)
...
# Netherlands 
nl09_lab <- lapply(nl09, attr, "label") 
...
colnames(ca09) <- tolower(colnames(ca09))
ca09 <- ca09 %>%
  mutate(
    degree = NA,
    cntry = 124,
...
ca09 <- dplyr::select(ca09, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, rincO, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, inclarge, isco88, isco88_2digit, fisco88_2digit, misco88_2digit, spisco88_2digit)


# Netherlands 1999
nl99_lab <- lapply(nl99, attr, "label") 
nl99 <- nl99 %>%
  mutate(
    degree = ifelse(degree %in% c(0,99), NA, degree-1),
    cntry = 528,
    year = 1999,
    wave = 1999,
    incdiff = v35,
...
nl99 <- dplyr::select(nl99, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, rincO, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, inclarge, isco88, isco88_2digit, fisco88_2digit, misco88_2digit, spisco88_2digit)


# Belgium 2009
colnames(socin09) <- tolower(colnames(socin09))
...
be09 <- socin09 %>%
  ...
  mutate(
    ...
    rincZ = ntile(be_rinc, 10),
    hincZ = ntile(be_inc, 10),
    rincO = be_rinc,
    wrkst = case_when(
        wrkst == 4 ~ 8,
        wrkst %in% c(97:99) ~ NA_real_,
        TRUE ~ as.numeric(wrkst)
      ),
    wrkhrs = case_when(
        wrkhrs > 59 & wrkhrs < 97 ~ 60,
        wrkhrs %in% c(97:99) ~ NA_real_,
        TRUE ~ as.numeric(wrkhrs)
      ),
    egp = occupar::isco88toEGP(isco88, n.employees = nemploy, self.employed = selfemp),
    egp6 = car::recode(egp, recodes = egp6),
    isei = occupar::isco88toISEI92(isco88),
  ...
  select(cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, rincO, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, inclarge, isco88, isco88_2digit, fisco88_2digit, misco88_2digit, spisco88_2digit)
...
fi09 <- socin09 %>%
  ...
  mutate(
    ...
    rincZ = ntile(fi_rinc, 10),
    hincZ = ntile(fi_inc, 10),
    rincO = fi_rinc,
    wrkst = case_when(
        wrkst == 4 ~ 8,
        wrkst %in% c(97:99) ~ NA_real_,
...
mer1 <- plyr::rbind.fill(dk99,ie99,ie87,nl87, nl99)
mer2 <- plyr::rbind.fill(mer1,nl09,ca09, be09, fi09)
...
# add alpha country labels - easier for readers to understand than numbers (i.e., USA = 840 or "USA")
df <- df %>%
  mutate(iso3c = countrycode(cntry, "iso3n", "iso3c")) %>%
  select(year, wave, country, iso3c, cntry, everything())
