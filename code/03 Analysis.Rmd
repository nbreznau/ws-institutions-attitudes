...
pratstws - Pension replacement rate standard worker single (pbenstws/gapwyear)
...
sied <- sied %>%
  ...
  select(iso3c, year, pratstws, urtsw26f, artsw26f, pcovratp, ucovratl, acovratl)

sied_plot <- sied %>%
  subset(iso3c %in% unique(df_macro$iso3c)) %>%
...
sied_plot <- sied_plot %>%
  group_by(year10) %>%
  mutate(pension_rr = mean(pratstws, na.rm = T),
...
pension <- sied_plot %>%
  ...
  annotate("text", x = 1980, y = 0.36, label = "Wage Replacement", hjust = 0) +
  theme_classic()
